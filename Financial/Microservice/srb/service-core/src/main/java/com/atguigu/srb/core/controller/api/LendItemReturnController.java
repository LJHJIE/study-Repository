...
public class LendItemReturnController {
    ...
    @ApiOperation("根据用户Id获取回款列表")
    @GetMapping("/auth/itemReturnList")
    public R itemList(HttpServletRequest request) {
        String token = request.getHeader("token");
        Long userId = JwtUtils.getUserId(token);
        List<LendItemReturn> list = lendItemReturnService.selectLendItemReturnListByInvestUserId(userId);
        return R.ok().data("itemReturnList", list);
    }
}
