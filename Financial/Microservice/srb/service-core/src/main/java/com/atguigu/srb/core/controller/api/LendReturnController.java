...
public class LendReturnController {
    ...
    @ApiOperation("根据用户Id获取还款计划列表")
    @GetMapping("/returnList")
    public R returnList(HttpServletRequest request) {
        String token = request.getHeader("token");
        Long userId = JwtUtils.getUserId(token);
        List<LendReturn> list = lendReturnService.selectLendReturnListByUserId(userId);
        return R.ok().data("returnList", list);
    }
}
