            ...
            <div class="fl quick-info">
              <div class="info-1">
                ...
                <span class="info1-input">
                  <input type="text" class="pay-money-txt" placeholder="请输入充值金额" maxlength="10" v-model="chargeAmt" />
                  <em>元</em>
                </span>
              </div>
  ...
  methods: {
    ...
    commitCharge() {
      ...
      if (this.chargeAmt == null) {
        ...
        return
      } else if (this.chargeAmt < 10) {
        this.$message.error('充值金额不能低于10元')
        return
      } else {
        this.$alert(
          ...
          '前往汇付宝资金托管平台',
          {
            dangerouslyUseHTMLString: true,//是否解析提示的html
            confirmButtonText: '立即前往',
            callback: (action) => {
              if (action === 'confirm') {
                this.$axios.$post('/api/core/userAccount/auth/commitCharge/' + this.chargeAmt)
                  .then((response) => {
